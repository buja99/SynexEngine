...
void DirectXCommon::Cleanup() {
	...
	if (copyRootSignature_) { copyRootSignature_.Reset(); }
	if (copyPipelineState_) { copyPipelineState_.Reset(); }

	if (grayscaleRootSignature_) { grayscaleRootSignature_.Reset(); }
	if (grayscalePipelineState_) { grayscalePipelineState_.Reset(); }
	if (grayscaleConstBuffer_) { grayscaleConstBuffer_.Reset(); }
